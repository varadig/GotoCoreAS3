...
public class CoreNet extends CoreBaseClass {
    ...
    private function serviceTest(params:Array):Promise {
        var deferred:Deferred = new Deferred();
        if (this.monitor.running)
            this.monitor.stop();

        this.monitor.pollInterval = (params[POLLING_INTERVAL]) ? params[POLLING_INTERVAL] : 1000;
        this.monitor.addEventListener(StatusEvent.STATUS, function (event:StatusEvent):void {
            deferred.resolve(monitor.available);
            monitor.stop();
        }, false, 0, true);
        this.monitor.start();
    ...
    private function serviceStop(params:Array):void {
        if (this.monitor.running)
            this.monitor.stop();
        if (this.monitor.hasEventListener(StatusEvent.STATUS))
            this.monitor.removeEventListener(StatusEvent.STATUS, announceStatus);
    ...
    private function serviceStart(params:Array):void {
        if (this.monitor.running)
            this.monitor.stop();
        if (!this.monitor.hasEventListener(StatusEvent.STATUS))
            this.monitor.addEventListener(StatusEvent.STATUS, announceStatus);
